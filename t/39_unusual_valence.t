...
is write_SMILES( [ $graph ] ), '[C]O[H]';

$graph->add_edge( $C, { symbol => 'H', number => 3 } );
is write_SMILES( [ $graph ] ), '[C](O[H])[H]';

$graph->add_edge( $C, { symbol => 'H', number => 4 } );
is write_SMILES( [ $graph ] ), '[C](O[H])([H])[H]';

$graph->add_edge( $C, { symbol => 'H', number => 5 } );
is write_SMILES( [ $graph ] ), 'C(O[H])([H])([H])[H]';

$graph->add_edge( $C, { symbol => 'H', number => 6 } );
is write_SMILES( [ $graph ] ), '[C](O[H])([H])([H])([H])[H]';
